...
class Authorization_Token 
{
    ...
    public function validateToken()
    {
            ...
            try
            {
                ...
                if(!empty($token_decode) AND is_object($token_decode))
                {
                    ...
                    if (empty($token_decode->API_TIME) OR !is_numeric($token_decode->API_TIME)) {
                        ...
                        return array('status' => FALSE, 'message' => 'Token Time Not Define!');
                    }
                    else
                    {
                        /**
                        ...
                        {
                            return array('status' => FALSE, 'message' => 'Token Time Expire.');
                        ...
                        }else
                        {
                             ...
                             */
                            return array('status' => TRUE, 'data' => $token_decode);
                        }
                    }
                    
                }else{
                    return array('status' => FALSE, 'message' => 'Forbidden');
                }
            }
            catch(Exception $e) {
                return array('status' => FALSE, 'message' => $e->getMessage());
        ...
        }else
        {
            ...
            return array('status' => FALSE, 'message' => $token_data['message'] );
        }
    }
    ...
    private function tokenIsExist($headers)
    {
        ...
        if(!empty($headers) AND is_array($headers)) {
            foreach ($headers as $header_name => $header_value) {
                // Adicionado : Verificação se header_value não é uma cadeia vazia
                if (strtolower(trim($header_name)) == strtolower(trim($this->token_header)) && !empty($header_value) ){
                    return array('status' => TRUE, 'token' => $header_value);
                }
            }
        }
        return array('status' => FALSE, 'message' => 'Token is not defined.');
    }
}